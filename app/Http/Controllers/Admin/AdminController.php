    ...
    public function home(){
        ...
        for ($i = 13; $i >= 7; $i--) {
            ...
            $_totalRevenue1 = Order::where('status', 4)
                ->whereDate('created_at', $date->format('Y-m-d'))
                ->sum('grand_total');
            $previousRevenueWeek[] = $_totalRevenue1;
        }
        $labelsWeek = [];
        for ($i = 6; $i >= 0; $i--) {
            $date = Carbon::now()->subDays($i);
        ...
        for ($i = 11; $i >= 6; $i--) {
            ...
            $_totalRevenue2 = Order::where('status', 4)
                ->whereBetween('created_at', [$startDate, $endDate])
                ->sum('grand_total');
            $previousRevenueMonth[] = $_totalRevenue2;
        }
        $labelsMonth = [];
        for ($i = 5; $i >= 0; $i--) {
        ...
        for ($i = 23; $i >= 12; $i--) {
            ...
            $_totalRevenue = Order::where('status', 4)
                ...
                ->whereMonth('created_at', $date->month)
                ->sum('grand_total');
            $previousRevenueYear[] = $_totalRevenue;
        }
        $labelsYear = [];
        for ($i = 11; $i >= 0; $i--) {
            $date = Carbon::now()->subMonths($i);
