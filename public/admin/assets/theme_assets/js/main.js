...
(function ($) {
  ...
  if (closeCompose !== null && closelabel !== null) {
    ...
    let start = moment('0000-01-01');
    let end = moment('9999-12-30');
    console.log('-------------------------------------------'+start);
    const urlParams = new URLSearchParams(window.location.search);
    const dateRangeParam = urlParams.get('date-ranger');
    if (dateRangeParam) {
        const [startDateStr, endDateStr] = dateRangeParam.split(' - ');
        start = moment(startDateStr, 'MM/DD/YYYY');
        end = moment(endDateStr, 'MM/DD/YYYY');
    }
  $('input[name="date-ranger"]').daterangepicker({
    ...
    ranges: {
        "All": [moment('0000-01-01'), moment('9999-12-30')],
      Today: [moment(), moment()],
